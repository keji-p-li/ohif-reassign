...
namespace {
    ...
    void testConfidencePropagationFillsNearbyLowIntensityHole() {
        const int width = 31;
        const int height = 11;
        std::vector<std::uint16_t> labels(width * height, 0);
        std::vector<float> intensities(width * height, 50.0f);

        for (int v = 4; v <= 6; ++v) {
            for (int u = 13; u <= 17; ++u) {
                intensities[idx(u, v, width)] = 10.0f;
            }
        }
        for (int v = 0; v < height; ++v) {
            for (int u = 25; u < width; ++u) {
                intensities[idx(u, v, width)] = 10.0f;
            }
        }
        ...
        input.intensities = intensities.data();
        input.intensityCount = int(intensities.size());
        input.positiveSeeds = {{5, 5}, {6, 5}, {7, 5}};
        input.negativeSeeds = {{28, 5}, {29, 5}, {30, 5}};

        const auto result = runReassignVoronoi2D(labels, input);

        require(result.classifiedVoxels > 0, "confidence propagation should grow a region");
        require(labels[idx(5, 5, width)] == 8, "positive seed should remain segment");
        require(labels[idx(15, 5, width)] == 8, "nearby low-intensity hole should be filled");
        require(labels[idx(29, 5, width)] == 0, "negative trace should remain a hard exclusion");
    }
...
int main() {
    try {
        ...
        testConfidencePropagationFillsNearbyLowIntensityHole();
        runPerformanceSmoke();
